...
export default class GameDataService {
  ...
  async mapping() {
    ...
    for (let i = 0; i < this.shuffledValue.length - 1; i += 1) {
      ...
      const partOfSpeech = test.results
        ? test?.results[0]?.partOfSpeech
        : auditionGameVariables.IDK;
      if (this.shuffledValue[i]?.wordTranslate) {
        wordsInfo.push({
          word: this.shuffledValue[i].word,
          wordTranslate: this.shuffledValue[i].wordTranslate,
          audio: this.shuffledValue[i].audio,
          partOfSpeech,
          image: this.shuffledValue[i].image,
          id: this.shuffledValue[i].id,
        });
    ...
    const filteredWordsInfo = wordsInfo
